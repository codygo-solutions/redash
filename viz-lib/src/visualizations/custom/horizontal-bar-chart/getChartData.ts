...
const prepareData = (rows: any, options: any) => {
  const entries = Object.entries(options.columnMapping)
  const xAxes = entries.filter(([_, val]) => val === 'x').map(([key]) => key)
  const yAxes = entries.filter(([_, val]) => val === 'y').map(([key]) => key)
  ...
  return xAxes.map((xAxis) => {
    return rows.reduce((acc: any, row: any) => {
      yAxes.forEach((yAxis) => {
        ...
        acc[yAxis] ??= {}
        acc[yAxis][xAxisValue] ??= 0
        const yAxisValue = Number(row[yAxis])
        if (!isNaN(yAxisValue)) acc[yAxis][xAxisValue] += yAxisValue
      })
      return acc
...
export default function getChartData (data: any, options: any) {
  ...
  console.log(series)
  if (options) return prepareData(data, options)
  return sortBy(values(series), ({ name }: any) => {
